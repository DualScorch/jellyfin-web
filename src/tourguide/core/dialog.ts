...
async function renderDialogHtml(tgInstance : TourGuideClient) {
    ...
    // let prevBtnClass = 'tg-dialog-btn emby-button';
    let prevBtnClass = 'emby-button raised customBtn';
    let prevDisabled = 'false';
    if (tgInstance.activeStep === 0) {
        prevDisabled = 'true';
        // prevBtnClass += ' disabled';
        prevBtnClass += ' disabledButton';
    }
    ...
    // if (tgInstance.options.showButtons && !tgInstance.options.hideNext) htmlRes += `<button type="button" class="tg-dialog-btn emby-button" id="tg-dialog-next-btn">${tgInstance.options.nextLabel}</button>`;
    if (tgInstance.options.showButtons && !tgInstance.options.hideNext) htmlRes += `<button type="button" class="emby-button raised customBtn" id="tg-dialog-next-btn">${tgInstance.options.nextLabel}</button>`;
    ...
    return new Promise((resolve, reject) => {
        ...
        if (backBtn) {
            if (tgInstance.activeStep === 0) {
                ...
                backBtn.classList.add('disabledButton');
                backBtn.setAttribute('disabled', 'true');
            } else {
                // backBtn.classList.remove('disabled');
                backBtn.classList.remove('disabledButton');
...
function computeDialogPosition(tgInstance : TourGuideClient) {
    ...
    return new Promise(async (resolve) => {
        ...
        const targetElem = tgInstance.tourSteps[tgInstance.activeStep].target as HTMLElement;
